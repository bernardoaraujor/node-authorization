...
pub mod pallet {
    ...
    pub enum Event<T: Config> {
        ...
        ConnectionsAdded {
            peer_id: PeerId,
            allowed_connections: Vec<PeerId>,
        },
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn add_well_known_node(
            ...
            T::AddOrigin::ensure_origin(origin)?;
                ...
                node.0.len() < T::MaxPeerIdLength::get() as usize,
                Error::<T>::PeerIdTooLong
            );

            let mut nodes = WellKnownNodes::<T>::get();
            ensure!(
                nodes.len() < T::MaxWellKnownNodes::get() as usize,
        ...
        pub fn swap_well_known_node(
            ...
            T::SwapOrigin::ensure_origin(origin)?;
                ...
                add.0.len() < T::MaxPeerIdLength::get() as usize,
                Error::<T>::PeerIdTooLong
            );
            ...
            Self::deposit_event(Event::NodeSwapped {
                removed: remove,
                added: add,
            });
            Ok(())
        ...
        pub fn claim_node(origin: OriginFor<T>, node: PeerId) -> DispatchResult {
            ...
            Self::deposit_event(Event::NodeClaimed {
                peer_id: node,
                who: sender,
            });
            Ok(())
        ...
        pub fn remove_claim(origin: OriginFor<T>, node: PeerId) -> DispatchResult {
            ...
            let owner = Owners::<T>::get(&node).ok_or(Error::<T>::NotClaimed)?;
                ...
                !WellKnownNodes::<T>::get().contains(&node),
        ...
        pub fn transfer_node(
            ...
            Self::deposit_event(Event::NodeTransferred {
                peer_id: node,
                target: owner,
            });
            Ok(())
